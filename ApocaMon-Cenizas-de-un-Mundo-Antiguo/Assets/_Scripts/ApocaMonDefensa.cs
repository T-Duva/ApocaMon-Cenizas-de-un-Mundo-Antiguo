...
/// Al nacer lee Mutante_Prueba y se pinta del tipo elegido por el jugador. Clic derecho para quitar/reordenar.
...
public class ApocaMonDefensa : MonoBehaviour
{
    ...
    [Tooltip("Asset Mutante_Prueba (DatosApocaMon) con el tipo elegido en la escena anterior.")]
    [SerializeField] private DatosApocaMon mutantePrueba;

    [Header("Pool de 10 tipos/colores (mismo orden que en Desfile para coincidir)")]
    [SerializeField] private TipoYColorDefensa[] poolTipos = new TipoYColorDefensa[TamanioPool];
    ...
    [System.Serializable]
    public struct TipoYColorDefensa
    {
    ...
    /// <summary>
    /// Lee el tipo de Mutante_Prueba y aplica el color del pool. Llamado al nacer en esta escena.
    /// </summary>
    private void PintarSegunMutantePrueba()
    {
        if (mutantePrueba == null || poolTipos == null || poolTipos.Length == 0)
            return;

        TipoApocaMon tipoElegido = mutantePrueba.tipoPrincipal;
        for (int i = 0; i < poolTipos.Length; i++)
        {
            if (poolTipos[i].tipo == tipoElegido)
            {
                if (visual != null)
                    visual.color = poolTipos[i].colorVisual;
                return;
            }
        }
